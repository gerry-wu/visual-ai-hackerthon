...
describe("UI Elements Test: ", () => {
  it("All elements should existed", () => {
    ...
    expect(LoginPage.username.getAttribute("placeholder")).to.equal(
      "Enter your username"
    );
    expect(LoginPage.usernameIcon.isExisting()).to.be.true;
    expect(LoginPage.passwordLabel.getText()).to.equal("Password");
    expect(LoginPage.password.getAttribute("placeholder")).to.equal(
      "Enter your password"
    );
...
describe("Data-driven Test", () => {
  ...
  it("should login successfully with username and password", () => {
    ...
    expect(browser.getUrl()).to.equal(
      `${browser.config.baseUrl}/hackathonApp.html`
    );
  });
});
...
describe("Table Sort Test", () => {
  it("should sort in ascending order", () => {
    ...
    const expectedData = originalData.sort(
      (a, b) => a.amountValue - b.amountValue
    );
